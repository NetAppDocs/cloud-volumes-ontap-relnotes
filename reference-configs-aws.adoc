...
pass:[*] Flash Cache is supported on m6id.xlarge, m6id.2xlarge, m6idn.xlarge, and m6idn.2xlarge; however, these instance types have fewer than 16 cores and are not recommended for high-performance or memory-intensive workloads. Under heavy load, they may restart due to memory constraints. To avoid this, use an instance type with a higher core count and more memory.

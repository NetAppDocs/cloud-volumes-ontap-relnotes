...
. ^5^ You can deploy the next-generation C3 family VMs on Cloud Volumes ONTAP 9.18.1 and later. The c3-standard-44 and c3-standard-88 machines and their LSSD variants support Tier-1 bandwidth in Google Cloud Console by default. With C3 VM types, you can't add a disk to an existing aggregate or switch to another variant within the C3 family. Refer to link:reference-configs-gcp.html#flash-cache-support[Flash Cache support] for more information on the LSSD variants.
...
Beginning with Cloud Volumes ONTAP 9.18.1, if you select the local SSD (LSSD) variants of the C3 VMs, Flash Cache gets enabled by default. Within your Cloud Volumes ONTAP deployments, you can switch LSSD VM types among different variants within the C3 family if that variant supports Flash Cache.

Check this table to understand the VM variant and the maximum number of LSSDs supported by them:

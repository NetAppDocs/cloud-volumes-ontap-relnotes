...
. The n1 series machines are no longer available for selection on BlueXP when deploying new instances of Cloud Volumes ONTAP in Google Cloud, and are only retained in old, existing systems. New Cloud Volumes ONTAP deployments in Google Cloud are supported from the 9.8 release. We recommend that you switch to the n2 series machine types that are supported by Cloud Volumes ONTAP 9.8 and later. 
+

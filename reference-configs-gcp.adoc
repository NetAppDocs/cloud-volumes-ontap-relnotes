...
. ##The minimum ONTAP version required for deploying Cloud Volumes ONTAP in Google Cloud is 9.8. Beginning with 9.8, you can deploy only the supported n2 series machine instances. n1 series machines are no longer supported in new deployments, and are retained in only old systems.##  

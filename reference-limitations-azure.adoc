...
Cloud Volumes ONTAP does not support Azure virtual machine (VM) extensions because they impact BlueXP management operations. If VM extensions are installed on your clusters, Cloud Volumes ONTAP disables them during deployment. If you have installed these extensions on your existing Cloud Volumes ONTAP VMs (enabled or disabled), NetApp strongly recommends contacting Microsoft Azure Support to remove them.

...
== Disk type limitations with HA configurations in multiple availability zones
Premium SSDv2 disks are not supported in high-availability (HA) deployments in non zonal regions. Note the following conditions for using Premium SSDv2 disks in Azure:

* The version of Cloud Volumes ONTAP is 9.15.1 or later.
* The HA configurations are in single availability Azure zones.
...
For information, refer to https://docs.netapp.com/us-en/cloud-volumes-ontap-9151-relnotes/reference-new.html#support-for-premium-ssd-v2-managed-disks-in-azure[Deploy HA pairs in single availability zones in Azure^].

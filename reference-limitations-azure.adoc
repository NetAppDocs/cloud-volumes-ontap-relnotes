...
== Limitations with using Azure VM Agent and extensions
Cloud Volumes ONTAP does not support Azure virtual machine (VM) extensions and the Azure Windows VM Agent (Azure VM Agent) in a deployment. Using Cloud Volumes ONTAP in Azure with VM extensions and the Azure VM Agent causes delays in operations and NetApp recommends avoiding them.

...
. ^2^ Use this VM only when Azure maintenance control is required. It is not recommended for other use cases due to higher pricing. When using Isolated instance types for Azure maintenance control, ensure that each VM in an HA pair has its own Maintenance Configuration in Azure with staggered schedules so that both VMs are not offline simultaneously.

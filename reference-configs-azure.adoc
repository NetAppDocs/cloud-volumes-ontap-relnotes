...
| Freemium 
| Optimized
| Capacity-based license (Essentials, Professional, and Edge Cache)

| Maximum system capacity
(disks + object storage) | 500 GiB | ? | 2 TiB 
...
| Supported disk types ^4^ 3+| Standard HDD Managed Disks, Standard SSD Managed Disks, and Premium SSD Managed Disks
...
| Maximum system capacity
(disks + object storage) | 500 GiB | 2 PiB | 2 TiB ^1^ | 10 TiB | 368 TiB | 368 TiB per license
...
. These VM types use an https://docs.microsoft.com/en-us/azure/virtual-machines/windows/disks-enable-ultra-ssd[Ultra SSD^] for VNVRAM, which provides better write performance.
+
If you choose any of these VM types when you deploy a new Cloud Volumes ONTAP system, you can't change to another VM type that _doesn't_ use an Ultra SSD for VNVRAM. Conversely, if you deployed Cloud Volumes ONTAP using any other VM type, you won't be able to change to a VM type that uses an Ultra SSD for VNVRAM. 
...
| Maximum system capacity
(disks + object storage) | 500 GiB | 2 PiB | 10 TiB | 368 TiB | 368 TiB per license
